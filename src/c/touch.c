...
// ---------------------------------------------------------------------------
// Compatibility shim for ported touch code
// ---------------------------------------------------------------------------

typedef enum {
  TouchTimerEffect_Remaining,
  TouchTimerEffect_Overtime
} TouchTimerEffect;
...
typedef struct {
  ...
  uint16_t touchLiftMinDurationMs;
  uint16_t touchMinDurationMs;
  bool touchDisableWhileInactive;
  uint8_t touchInputTimeoutDeciseconds;
} AppConfig;

static AppConfig s_touch_compat_config = {
  .ringColorRemaining = PBL_IF_COLOR_ELSE(GColorGreen, GColorWhite),
  .ringColorOvertime = PBL_IF_COLOR_ELSE(GColorGreen, GColorWhite),
  .touchTimerMode = TouchTimerEffect_Remaining,
  .touchLiftMinDurationMs = 50,
  .touchMinDurationMs = 100,
  .touchDisableWhileInactive = true,
  .touchInputTimeoutDeciseconds = 10
};

static AppConfig *config_get(void) {
  return &s_touch_compat_config;
}

static void graphics_color_circle(GContext *ctx, GPoint center, uint16_t radius, GColor color) {
...
static TextLayer* s_layer_time_text = NULL;
static char s_central_text[MAX_TEXT_SIZE] = "hello";
static char s_time_text[MAX_TEXT_SIZE] = "00:00";
...
static inline GColor color_inner_bg(void) {
    return PBL_IF_COLOR_ELSE(GColorMintGreen, GColorWhite);
}
...
#define INDEX_LINES 0
        ...
        const char* text = ((s_select_mode == SELECTMODE_HOUR) ? hours_text[i] : mins_text[i]);
...
static void update_selection_text(void) {
  if (s_selected_hours <= 0) {
    ...
    text_layer_set_font(s_layer_central_text, font);
    GRect bounds = layer_get_bounds(text_layer_get_layer(s_layer_central_text));
    const int16_t main_text_h = 36 + 8;
    layer_set_bounds(text_layer_get_layer(s_layer_central_text), GRect(0, grect_center_point(&bounds).y - (main_text_h / 2), bounds.size.w, main_text_h));
    const char* fmt = "%02d:%02d";
    snprintf(s_central_text, sizeof(s_central_text), fmt, s_selected_minutes, s_selected_seconds);
  } else {
    GFont font = fonts_get_system_font(FONT_KEY_LECO_26_BOLD_NUMBERS_AM_PM);
    text_layer_set_font(s_layer_central_text, font);
    GRect bounds = layer_get_bounds(text_layer_get_layer(s_layer_central_text));
    const int16_t main_text_h = 26 + 8;
    layer_set_bounds(text_layer_get_layer(s_layer_central_text), GRect(0, grect_center_point(&bounds).y - (main_text_h / 2), bounds.size.w, main_text_h));
    const char* fmt = "%d:%02d:%02d";
    snprintf(s_central_text, sizeof(s_central_text), fmt, s_selected_hours, s_selected_minutes, s_selected_seconds);
  }
    text_layer_set_text_color(s_layer_central_text, color_inner_fg());
    text_layer_set_text(s_layer_central_text, s_central_text);

    const char* explanation = "Timer";
    ...
    // Display the end time with the current time + selected time, hh:mm
    time_t end_time_s = time(NULL) + (s_selected_hours * 3600) + (s_selected_minutes * 60) + (s_selected_seconds);
    tm end_time_tm = *localtime(&end_time_s);
    strftime(s_time_text, sizeof(s_time_text), clock_is_24h_style() ? "%H:%M" : "%I:%M", &end_time_tm);
    if (s_time_text[0] == '0') { // Strip leading zero
      for (uint8_t ii = 1; ii < ARRAY_LENGTH(s_time_text); ii++) {
        s_time_text[ii - 1] = s_time_text[ii];
      }
    }
    text_layer_set_text_color(s_layer_time_text, color_inner_fg());
    text_layer_set_text(s_layer_time_text, s_time_text);
}
        ...
        if (s_touch_area == TOUCH_AREA_OUTER) {
            if (is_touch_state_long_enough(config_get()->touchMinDurationMs)) {
                ...
                if (s_select_mode == SELECTMODE_HOUR) {
                    ...
                    s_callback(s_selected_hours, s_selected_minutes, s_selected_seconds);
                    finish();
                }
...
void touch_create(Layer* parent, TouchSelectionCallback callback) {
    ...
    if (touch_service_is_enabled() && (s_layer == NULL)) {
        ...
        const GFont main_text_font = fonts_get_system_font(FONT_KEY_LECO_38_BOLD_NUMBERS);
        const int16_t main_text_h = 36 + 8;
        ...
        s_layer_explanation_text = text_layer_create(
            GRect(0, grect_center_point(&bounds).y - (main_text_h / 2) - 18 - 8, bounds.size.w, 18));
        text_layer_set_text(s_layer_explanation_text, "");
        text_layer_set_text_alignment(s_layer_explanation_text, GTextAlignmentCenter);
        text_layer_set_font(s_layer_explanation_text, fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD));
        ...
        s_layer_time_text = text_layer_create(
            GRect(0, grect_center_point(&bounds).y + (main_text_h / 2), bounds.size.w, 24));
        text_layer_set_text(s_layer_time_text, "");
        text_layer_set_text_alignment(s_layer_time_text, GTextAlignmentCenter);
        text_layer_set_font(s_layer_time_text, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD));
        text_layer_set_background_color(s_layer_time_text, GColorClear);
        layer_add_child(s_layer, (Layer*)s_layer_time_text);
        layer_set_hidden(s_layer, true);
...
void touch_destroy(void) {
    ...
    if (s_layer != NULL) {
        ...
        text_layer_destroy(s_layer_time_text);
        cancel_animation_timer();
        cancel_timeout();
    }
}
